...
| **boxkit** | GUI apps with no acceptable Fedora source (no maintained COPR/RPM, and Dimitri refuses Flatpak/AppImage as primary delivery). Produces `udx-toolbox` (Arch), `ubuntu-gui-toolbox` (Ubuntu 24.04, pinned), and `playwright-toolbox` (Ubuntu). | `ghcr.io/dpietersz/udx-toolbox` (Storage Explorer, Obsidian, Polypane, Bruno, Legcord, LibreOffice, darktable), `ghcr.io/dpietersz/ubuntu-gui-toolbox` (LocalSend — needs older glibc), `ghcr.io/dpietersz/playwright-toolbox` |
...
1. **Adding a CLI tool** → `dot_config/mise/config.toml.tmpl` (preferred) or a brew bundle file. Never add via `rpm-ostree`. Never add it to `bluefin-udx` unless it needs `sudo` or has to exist before chezmoi runs.
   - **npm-published CLIs** (no brew/mise option, e.g. `@getpaseo/cli`, `@augmentcode/auggie`, `mcp-hub`) → append the package name to both the latest-version comment block and `BUN_GLOBAL_PKGS` array in `.chezmoiscripts/run_onchange_after_02a-install-bun-globals.sh.tmpl`. **Always use `bun install -g`, never `npm install --prefix`/`npx`.** Bun globals land in `~/.bun/bin/` (already on PATH via `dot_config/shell/10-path.sh.tmpl:21`); upstream latest detection is centralized in `.chezmoitemplates/bun-npm-version.tmpl` so the `run_onchange_` script re-runs when packages publish. Do not create a new per-package installer script — the existing one is the single point of entry.
2. **Adding a GUI app**:
...
## Three non-obvious traps (don't break these)

- **hyprlock/swayidle idle stack runs on ALL niri (Bluefin) machines; fingerprint is just an optional auth method.** `niri-idle-daemon.sh` (a swayidle supervisor: power-aware screen-off → hyprlock → suspend) is spawned unconditionally from the niri config, hyprlock is the locker everywhere via `Ctrl+Alt+L` and the idle chain, and noctalia's own idle behaviors are disabled in `dot_config/noctalia/config.toml.tmpl` — all ungated so both laptops behave identically (P14s with a sensor, T580 without). `hyprlock`+`swayidle` are baked into **both** bluefin-udx images (common.yml), and `ydotool`/`uinput uaccess` come from base bluefin, so no per-machine install is needed. The ONLY fingerprint-gated pieces are: `run_once_after_10-enable-fingerprint-pam` (layers `fprintd`/`fprintd-pam`, `authselect with-fingerprint`) and the `fingerprint { }` block + placeholder text in `hyprlock.conf.tmpl`. Without a sensor, hyprlock falls back to password-only PAM (`/etc/pam.d/system-auth`). Deploy of `.config/hypr/` + the idle scripts is gated on `.isBluefin` in `.chezmoiignore.tmpl`. Suspend is owned by `niri-lock.sh`'s lock-scoped timer, NOT swayidle (niri freezes idle while ext-session-locked — niri#2006 — so a swayidle-driven suspend fires at unlock and cascades; see the script headers). Idle timeouts are power-aware (battery 5/10/20 min blank/lock/suspend, AC 15/30/60). `power-profile-auto.sh` (also spawned from the niri config) auto-switches the power profile on AC↔battery transitions (battery→power-saver, AC→balanced) by setting `net.hadess.PowerProfiles` (tuned-ppd backend; `powerprofilesctl` isn't installed), keeping the noctalia widget in sync. **Hibernation/S4 was deliberately NOT implemented** — NVIDIA-resume fragility on an atomic, auto-updating image outweighs the gain; the P14s Gen5 has no S3 (firmware: `supports S0 S4 S5`, s2idle only), the T580 has native S3, and lid-close already does the right thing via logind defaults (`HandleLidSwitchDocked=ignore` for clamshell + external monitor).
- **LocalSend lives inside `ubuntu-gui-toolbox`, not on the host — and specifically NOT in `udx-toolbox`.** `run_onchange_after_17-configure-localsend` deep-merges managed `flutter.ls_*` keys into the host-side `shared_preferences.json`; `run_once_after_09-configure-firewall-localsend` opens port 53317 on the host firewall. Don't move the binary; the config and firewall hole live here, the binary lives in the boxkit image. **Why its own Ubuntu box:** LocalSend ships a Flutter binary built against an Ubuntu LTS toolchain and upstream hasn't rebuilt since v1.17.0 (Feb 2025). On the rolling Arch `udx-toolbox` (glibc 2.43) its Dart VM fails to initialise — the GTK window and titlebar map, nothing ever paints, and **stderr stays completely empty**, so it presents as an unusable black window with zero diagnostics. The identical binary runs fine on Ubuntu 24.04 (glibc 2.39). This is *not* NVIDIA, *not* the GPU, *not* the app config, and *not* the host image — all four were tested and cleared. If you ever move LocalSend back to the Arch box, the black window returns.
- **Helium's Widevine is donated by Chromium — don't remove Chromium from the image.** Helium (baked in bluefin-udx) ships without Google's Widevine CDM, so Spotify's web player is silent (it hard-requires EME; YouTube etc. don't). `run_after_15-sync-helium-widevine` mirrors Chromium's self-updated CDM (`~/.config/chromium/WidevineCdm`) into Helium's user config (`~/.config/net.imput.helium/WidevineCdm`) on every apply, falling back to extracting it from the Google Chrome RPM only on a brand-new machine where Chromium hasn't run yet. The CDM lives in `~/.config` (user-scope, frequently updated) on purpose — baking it into the immutable image would go stale. If Chromium is ever dropped from bluefin-udx, the free donor path dies and only the slow RPM bootstrap remains, so keep Chromium baked (or add a system Widevine package as the donor). Same binary-in-image / config-in-dotfiles split as LocalSend above.
