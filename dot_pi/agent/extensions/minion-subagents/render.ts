...
export function renderSubagentResult(
	...
	if (d.mode === "single" && d.results.length === 1) {
		...
		const traitsInfo = r.traits?.length ? theme.fg("dim", ` (${r.traits.join(", ")})`) : "";
		c.addChild(new Text(truncLine(`${icon} ${theme.fg("toolTitle", theme.bold(r.agent))}${traitsInfo}${progressInfo}`, w), 0, 0));
	...
	for (let i = 0; i < stepsToShow; i++) {
		...
		if (r.traits?.length) {
			c.addChild(new Text(truncLine(theme.fg("dim", `    traits: ${r.traits.join(", ")}`), w), 0, 0));
		}
