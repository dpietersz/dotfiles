...
export function executeAsyncChain(
	...
	const { chain, agents, ctx, cwd, task: originalTask, maxOutput, artifactsDir, artifactConfig, shareEnabled, sessionRoot } = params;
	...
	const buildSeqStep = (s: SequentialStep) => {
		...
		const rawTask = (s.task ?? "{previous}").replace(/\{task\}/g, originalTask ?? "");
		const task = injectSingleOutputInstruction(rawTask, outputPath);
