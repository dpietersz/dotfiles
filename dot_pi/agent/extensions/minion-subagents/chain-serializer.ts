	...
	for (const line of configLines) {
		...
		if (key === "traits") {
			const traits = rawValue
				...
				.map((v) => v.trim())
				.filter(Boolean);
			if (traits.length > 0) step.traits = traits;
		}
	}
...
/**
 * Find positions of all code fences (``` blocks) in text.
...
function findCodeFenceRanges(text: string): Array<[number, number]> {
	...
	// If there's an unclosed fence, treat everything from it to end as fenced
	if (openStart !== null) {
...
function isInsideCodeFence(offset: number, ranges: Array<[number, number]>): boolean {
...
export function parseChain(content: string, source: "user" | "project", filePath: string): ChainConfig {
	...
	const codeFenceRanges = findCodeFenceRanges(body);
	const allMatches = [...body.matchAll(/^##\s+(.+)[^\S\n]*$/gm)];
	// Filter out ## headers that appear inside code fences
	const matches = allMatches.filter((m) => !isInsideCodeFence(m.index!, codeFenceRanges));
...
export function serializeChain(config: ChainConfig): string {
	...
	for (let i = 0; i < config.steps.length; i++) {
		...
		if (Array.isArray(step.traits) && step.traits.length > 0) lines.push(`traits: ${step.traits.join(", ")}`);
		lines.push("");
