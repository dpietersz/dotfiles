...
import { composeSystemPrompt } from "./compose-traits.js";
...
export async function runSync(
	...
	// When explicit skills are specified, disable pi's own skill discovery
	// so the spawned process doesn't inject the full <available_skills> catalog.
	if (skillNames.length > 0) {
		args.push("--no-skills");
	}

	// Compose system prompt: agent context + traits (if provided) + skills
	let systemPrompt = agent.systemPrompt?.trim() || "";
	if (options.traits && options.traits.length > 0) {
		systemPrompt = composeSystemPrompt(agent.name, systemPrompt, options.traits);
	}
	...
	const result: SingleResult = {
		...
		traits: options.traits?.length ? options.traits : undefined,
	};
