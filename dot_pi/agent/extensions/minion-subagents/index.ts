...
import { findChains, handleManagementAction } from "./agent-management.js";
import registerSubagentNotify from "./notify.js";
...
• SAVED CHAIN: { chainName: "name", task: "..." } - run a saved .chain.md file by name
• PARALLEL: { tasks: [{agent,task}, ...] } - concurrent execution

TRAIT COMPOSITION: Add traits to any agent for behavioral specialization.
Pick agent by capability, add 1-3 traits to shape behavior for the task.
Example: { agent: "scout", traits: ["security", "skeptical"], task: "audit auth module" }
...
Saved chain: { chainName: "refine-linear-issue", task: "Create issue for auth refactor" }
...
• Use chainName instead of agent for chain operations`,
		...
		async execute(_id, params, signal, onUpdate, ctx) {
			...
			// Resolve chainName to chain steps array (execution mode)
			if (params.chainName && !params.action && !params.chain) {
				const chainMatches = findChains(params.chainName, ctx.cwd, "both");
				if (chainMatches.length === 0) {
					const available = discoverAgentsAll(ctx.cwd).chains.map((c) => c.name);
					return {
						content: [{ type: "text", text: `Chain '${params.chainName}' not found. Available: ${available.join(", ") || "none"}` }],
						isError: true,
						details: { mode: "chain" as const, results: [] },
					};
				}
				const chainConfig = chainMatches[chainMatches.length - 1]!; // highest priority (project > user)
				params.chain = chainConfig.steps.map((step) => ({
					agent: step.agent,
			...
			if (hasTasks && params.tasks) {
				...
				const results = await mapConcurrent(params.tasks, MAX_CONCURRENCY, async (t, i) => {
					...
					return runSync(ctx.cwd, agents, t.agent, tasks[i]!, {
						...
						traits: (t as { traits?: string[] }).traits,
						onUpdate: onUpdate
							? (p) => {
		...
		renderCall(args, theme) {
			...
			const traitsLabel = args.traits?.length
				? theme.fg("dim", ` (${args.traits.join(", ")})`)
				: "";
			return new Text(
				`${theme.fg("toolTitle", theme.bold("subagent "))}${theme.fg("accent", args.agent || "?")}${traitsLabel}${asyncLabel}`,
				0,
				0,
			);
		},
	...
	pi.registerCommand("chain", {
		description: "Run a saved chain or agents in sequence: /chain chainName \"task\" or /chain scout \"task\" -> planner [--bg]",
		getArgumentCompletions: (prefix: string) => {
			// Autocomplete both saved chain names and agent names
			const { chains } = discoverAgentsAll(baseCwd);
			const agents = discoverAgents(baseCwd, "both").agents;
			const chainItems = chains.map((c) => ({ value: c.name, label: `${c.name} (chain)` }));
			const agentItems = agents.map((a) => ({ value: a.name, label: a.name }));
			const all = [...chainItems, ...agentItems];
			if (!prefix) return all.length > 0 ? all : null;
		...
		handler: async (args, ctx) => {
			...
			// Check if first token is a saved chain name
			const trimmed = cleanedArgs.trim();
			const firstToken = trimmed.split(/\s+/)[0] ?? "";
			const chainMatches = findChains(firstToken, ctx.cwd, "both");
			if (chainMatches.length > 0 && !trimmed.includes(" -> ")) {
				// Saved chain mode: /chain chainName "task" or /chain chainName -- task
				const rest = trimmed.slice(firstToken.length).trim();
				let task: string;
				const qMatch = rest.match(/^(?:"([^"]*)"|'([^']*)')$/);
				if (qMatch) {
					task = (qMatch[1] ?? qMatch[2]) || "";
				} else if (rest.startsWith("-- ")) {
					task = rest.slice(3).trim();
				} else {
					task = rest;
				}
				if (!task) {
					ctx.ui.notify(`Usage: /chain ${firstToken} "task description"`, "error");
					return;
				}
				const params: Record<string, unknown> = { chainName: firstToken, task, clarify: false, agentScope: "both" };
				if (bg) params.async = true;
				pi.sendUserMessage(`Call the subagent tool with these exact parameters: ${JSON.stringify(params)}`);
				return;
			}

			// Ad-hoc chain mode: /chain scout "task" -> planner
	...
	// Register the notify extension (handles background task completion notifications)
